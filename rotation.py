...
def quat2mat(x):
    ...
    x2 = x[2]
    x3 = x[3]
    if LA.norm(x[1:]) < 1.0e-10:
        return np.eye(3)
    else:
        x = x / LA.norm(x)
        return 2 * np.array(
            [
                [x0**2 + x1**2 - 0.5, x1 * x2 - x3 * x0, x1 * x3 + x2 * x0],
                [x2 * x1 + x3 * x0, x0**2 + x2**2 - 0.5, x2 * x3 - x1 * x0],
                [x3 * x1 - x2 * x0, x3 * x2 + x1 * x0, x0**2 + x3**2 - 0.5],
            ]
        )
    # v = np.array([x1, x2, x3])
...
def exp(u):
    if LA.norm(u) < 1e-20:
        ...
        M = (
            np.eye(3)
            + np.sin(theta) * chap(v)
            + 2 * np.sin(theta / 2) ** 2 * (LA.matrix_power(chap(v), 2))
        )
    return M
...
def T(u):
    if LA.norm(u) < 1e-20:
        ...
        e = np.array(
            [u / LA.norm(u)]
        )  # = vecteur unitaire directeur de la rotation incrémentale
        ...
        T = (
            ...
            + (1 / x1) * (np.eye(3) - np.dot(np.transpose(e), e))
            + 0.5 * chap(u)
        )
    return T
...
def quat2vect(q):
    ...
    if LA.norm(q[1:]) < 1e-20:
        ...
        v = theta * (
            np.array(q[1:]) / LA.norm(np.array(q[1:]))
        )  # angle*vecteur unitaire
    return v
...
def quat2vect2(q):
    ...
    if LA.norm(q[1:]) < 1e-20:
        ...
        # theta = 2*np.arcsin(LA.norm(np.array(q[1:]))) # angle de rotation
        theta = 2.0 * np.arctan2(LA.norm(np.array(q[1:])), q[0])  # angle de rotation
        v = theta * (
            np.array(q[1:]) / LA.norm(np.array(q[1:]))
        )  # angle*vecteur unitaire
    return v
...
def quat2mat_df(df, **kwargs):
    ...
    x = np.array(
        [df[kwargs["q1"]], df[kwargs["q2"]], df[kwargs["q3"]], df[kwargs["q4"]]]
    ).astype(float)
    x0 = x[0]
    x1 = x[1]
    x2 = x[2]
